...
vignette: >
  %\VignetteIndexEntry{Initializing a dashboard}
...
As mentioned in the [umbrella documentation](https://folkehelseinstituttet.github.io/dashboards/umbrella.html#internalfilestructure), we expect that for the automated analysis `ANALYSIS`, there is an R package called `ANALYSIS` and the following folder structure exists on the system:
...
[fhi::DashboardInitialiseOpinionated](../reference/DashboardInitialiseOpinionated.html) is always the first function called in a [RunProcess.R](https://folkehelseinstituttet.github.io/dashboards/rpackages.html#RunProcess) file. Its main purpose is to properly initialise the environment `fhi::PROJ`.
...
According to the [code style guidelines](https://folkehelseinstituttet.github.io/dashboards/contributingquickstart.html#code-style) we do not use `print`. We instead use `message`, `warning`, and `stop`. 
